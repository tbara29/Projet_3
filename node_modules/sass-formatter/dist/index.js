...
var SassFormatter = /** @class */ (function () {
    ...
    SassFormatter.Format = function (text, config) {
        ...
        for (STATE.char; STATE.char < STATE.text.length; STATE.char++) {
            var char = STATE.text[STATE.char];
            var last = STATE.char === STATE.text.length - 1;
            var isNewLine = char === '\n';
            if (isNewLine || last) {
                ...
                this.handleLine(new sassTextLine_1.SassTextLine(STATE.lineText, STATE.line), STATE);
            ...
            if (suf_regex_1.isIgnore(line.get())) {
                ...
                if (line.isEmptyOrWhitespace ||
                    ...
                    STATE.setLocalContext(__assign(__assign({}, format_utility_1.FormatHandleLocalContext(line, STATE)), { ResetTabs: suf_regex_1.isReset(line.get()), isAnd_: suf_regex_1.isAnd(line.get()), isProp: suf_regex_1.isProperty(line.get()), indentation: utility_1.getIndentationOffset(line.get(), STATE.CONTEXT.tabs, STATE.CONFIG.tabSize), isAdjacentSelector: suf_regex_1.isAdjacentSelector(line.get()), isHtmlTag: suf_regex_1.isHtmlTag(line
                            .get()
                            .trim()
                            .split(' ')[0]), isClassOrIdSelector: suf_regex_1.isClassOrId(line.get()), isInterpolatedProp: suf_regex_1.isInterpolatedProperty(line.get()) }));
                    // ####### Is @forward or @use #######
    ...
    SassFormatter.handleEmptyLine = function (STATE, line) {
        ...
        if (STATE.CONFIG.deleteEmptyRows && !STATE.CONTEXT.isLastLine) {
            var getNextLine = function () {
                for (var i = STATE.char + 1; i < STATE.text.length; i++) {
                    var char = STATE.text[i];
                    if (char === '\n') {
                        return new sassTextLine_1.SassTextLine(STATE.lineText, STATE.line + 1);
                    }
                    STATE.lineText += char;
                }
                return new sassTextLine_1.SassTextLine('', -1);
            };
    ...
    SassFormatter.isBlockHeader = function (line, STATE) {
        return (!STATE.LOCAL_CONTEXT.isInterpolatedProp &&
            (suf_regex_1.isMixin(line.get()) || // also adds =mixin
    ...
    SassFormatter.isProperty = function (STATE, lineText) {
        return (suf_regex_1.isInclude(lineText) || // adds +mixin, @include is handled in the block header.
